...
void CGameFramework::OnProcessingKeyboardMessage(HWND hWnd, UINT nMessageID, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_KEYUP:
		...
		case VK_F8:
			break;
		case VK_F9:
			ChangeSwapChainState();	//“F9” 키가 눌려지면 윈도우 모드와 전체화면 모드의 전환을 처리한다.
			break;
		default:
			break;
		}
...
void CGameFramework::ProcessInput()
{
	...
	if ((dwDirection != 0) || (cxDelta != 0.0f) || (cyDelta != 0.0f)) {
		...
		if (dwDirection) m_pPlayer->Move(dwDirection, 2500.0f * m_GameTimer.GetTimeElapsed(), true);
	}
...
//한 프레임 진행하기(ProcessInput()와 AnimateObjects()함수가 포함되어 있다)
