...
void CMesh::Render(ID3D12GraphicsCommandList* pd3dCommandList) {
	...
	if (m_pd3dIndexBuffer) {	//인덱스 버퍼가 있을 경우 인덱스 버퍼를 파이프라인에 연결해 렌더링
	...
	float fWidth, float fHeight, float fDepth) : CMesh(pd3dDevice, pd3dCommandList) 
{
	...
	std::uniform_real_distribution<float> urd(0, 1.0f);
	#define RANDOM_COLOR XMFLOAT4(urd(dre), urd(dre), urd(dre), urd(dre))
...
XMFLOAT3 CHeightMapImage::GetHeightMapNormal(int x, int z) {
	...
	//(x, z), (x+1, z), (x, z+1)의 픽셀에서 지형의 높이를 구한다. 
...
float CHeightMapImage::GetHeight(float fx, float fz) {
	/*지형의 좌표 (fx, fz)는 이미지 좌표계이다. 높이 맵의 x-좌표와 z-좌표가 높이 맵의 범위를 벗어나면 
	지형의 높이는 0이다.*/
